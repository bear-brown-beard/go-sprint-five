...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
    // ваш код здесь
    walkingCalories := ((walkingCaloriesWeightMultiplier * weight + (math.Pow(meanSpeed(action, duration), 2.0) / height) * walkingSpeedHeightMultiplier * weight) * duration * minInH)
